...
def itemorder():
    ...
    if request.method == 'POST': # Did Not Add In Cart Anything Yet
        ...
        session['chosenrestID'] = chosenID # Save For Future Uses.
        conn.close()
    return render_template('itemorder.html', user=customer_data, itemschosen=trueitemschosen, restaurantchosen=truerestaurantchosen)
...
def deleteproduct():
    if request.method == 'POST':
        ...
        for item in session['shoppingcart'].items(): # Search For Item To Delete
            if item[0] == itemtodelete:
                session['shoppingcart'].pop(item[0], None)
                if 'shoppingcart' in session: # If The Cart Is Still Not Empty, Update
                    for key, value in session['shoppingcart'].items(): # Technically Still Viable to DECREASE Total Price.
                ...
                break # If Cart Empty
        if totalquantity == 0:
            session.pop('shoppingcart') # Subject to Change.
        else:
...
def paymentconfirm():
    ...
    if request.method == 'POST':
        ...
        # Balance Sufficient, Proceed With Payment.
        # Check If Order Table Has Entries, To Setup 'OrderID'.
        cursor.execute('SELECT OrderID FROM Orders WHERE OrderID = (SELECT MAX(OrderID) FROM Orders)')
        OrderID = cursor.fetchone()
        if OrderID is None:
            OrderIDCount = 1
        else:
        ...
        for key, value in session['shoppingcart'].items():
            ...
            cursor.execute('INSERT INTO OrderItems (OrderID, ItemID, Quantity, Price) VALUES (?, ?, ?, ?)', (OrderIDCount, itemID, quantity, priceofitem))
        # Insert Order
        cursor.execute('INSERT INTO Orders (OrderID, CustomerID, RestaurantID, Notes, TotalPrice, CreatedAt) VALUES (?, ?, ?, ?, ?, ?)', (OrderIDCount, customer_data['CustomerID'], session['chosenrestID'], NotesToAdd, session['total_price'], datetime.now()))
        conn.commit()
        conn.close()
