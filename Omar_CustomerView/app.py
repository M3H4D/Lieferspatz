...
# Example of a Blueprint style: customerview_bp.route = Blueprint('customerview', __name__)
...
with app.app_context():
    if not path.exists('DBProject/' + DATABASE): # Change the Directory to Your Main Folder
        create_table()
...
def dashboard():
    ...
    cursor.execute('SELECT * FROM restaurants WHERE RestaurantID IN (SELECT RestaurantID FROM zipcodes WHERE ZipCode = ?)', (user['ZipCode'],))
    restaurantsclose = cursor.fetchall()
    conn.close()
    return render_template('dashboard.html', user=user, restaurantsclose=restaurantsclose)
    # New Part In Dashboard End

# Completely New Route, Change '@app.route' to suit the Blueprint
...
def itemorder():
    ...
    if request.method == 'POST':
        ...
        conn.close()
    return render_template('itemorder.html', user=user, itemschosen=itemschosen, restaurantchosen=restaurantchosen)
# End of New Route
